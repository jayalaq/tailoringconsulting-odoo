...
class AccountAnalyticLine(models.Model):
    ...
    def get_views(self, views, options=None):
        """
            Add filters to the search view `hr_timesheet.hr_timesheet_line_search` because the traditional way generates an error.
            The model `timesheets.analysis.report` uses this view as a template and does not have these new extra hours fields.
            NOTE: It is not necessary to add the module `sale_timesheet` as a dependency.
        """
        res = super(AccountAnalyticLine, self).get_views(views, options)
        if 'search' in res['views']:
            filter_replacements = {
                '<filter name="extra_hours" invisible="true"/>': '<filter name="extra_hours" string="Horas Extras" domain="[(\'extra_hours\', \'=\', True)]"/>',
                '<filter name="non_extra_hours" invisible="true"/>': '<filter name="non_extra_hours" string="Sin Horas Extras" domain="[(\'extra_hours\', \'=\', False)]"/>',
                '<filter name="non_validated_extra_hours" invisible="true"/>': '<filter name="non_validated_extra_hours" string="Horas Extras sin validar" domain="[(\'is_validate_extra_hour\', \'=\', False)]"/>',
                '<filter name="validated_extra_hours" invisible="true"/>': '<filter name="validated_extra_hours" string="Horas Extras validadas" domain="[(\'is_validate_extra_hour\', \'!=\', False)]"/>',
                '<separator invisible="true"/>': '<separator/>'
            }
            for old_filter, new_filter in filter_replacements.items():
                res['views']['search']['arch'] = res['views']['search']['arch'].replace(old_filter, new_filter)
        return res
