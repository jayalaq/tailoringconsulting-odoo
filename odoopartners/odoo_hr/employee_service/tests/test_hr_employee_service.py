...
class TestHrEmployeeService(common.TransactionCase):
    ...
    def test_function_hr_employee(self):
        employee = self.SudoEmployee.create({
            'name': 'Employee #8',
            'service_hire_date': '2024-01-31',
            'service_start_date' :'2024-01-31',
            'service_termination_date':'2024-02-27'
        })
        
        employee._onchange_service_hire_date()
        self.assertEqual(employee.service_start_date, employee.service_hire_date)
        self.assertEqual(employee.service_hire_date.strftime('%Y-%m-%d'), '2024-01-31')
