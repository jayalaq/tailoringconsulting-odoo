...
class HrEmployee(models.Model):
    ...
    def _compute_service_hire_date(self):
        for record in self:
            valid_contracts = record.contract_ids.filtered(
                lambda contract: contract.state not in ['cancel']
            ).sorted(key=lambda contract: contract.date_start)
            
            if valid_contracts:
                record.service_hire_date = valid_contracts[0].date_start
            else:
