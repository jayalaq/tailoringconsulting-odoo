...
class PurchaseOrder(models.Model):
    ...
    def _get_view(self, view_id=None, view_type='form', **options):
        ...
        if view_type in ('form'):
            tags = [('field', 'active_and')]
            countries = [self.env.ref('base.pe')]
            arch, view = self.env['base']._tags_invisible_per_country(arch, view, view_type, tags, countries)
        return arch, view
    ...
    def button_confirm(self):
        if self.active_and == 'done' or (self.company_id and self.company_id.country_id and self.company_id.country_id.code != 'PE'):
