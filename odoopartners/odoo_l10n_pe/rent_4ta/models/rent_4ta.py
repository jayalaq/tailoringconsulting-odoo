...
class Rent4taFiles(models.Model):
    ...
    def _compute_display_name(self):
        for rec in self:
            if rec.date_from and rec.date_to:
                rec.display_name = '%s - %s' % (rec.date_from.strftime('%d/%m/%Y'), rec.date_to.strftime('%d/%m/%Y'))
            else:
                rec.display_name = ''
    ...
    def generate_data_report_rent_4ta(self):
        ...
        for move_id in account_move_ids_list:
            ...
            if move_id.tax_totals:
                tax_totals = move_id.tax_totals
                if isinstance(tax_totals, str):
                    try:
                        tax_totals = json.loads(tax_totals)
                    except json.JSONDecodeError:
                        print(f"Error decoding JSON for move_id {move_id.id}")
                        continue
                
                if 'groups_by_subtotal' in tax_totals:
                    for line in tax_totals['groups_by_subtotal'].get('Untaxed Amount', []):
            ...
            if move_id.invoice_payments_widget:
                invoice_payments_widget = move_id.invoice_payments_widget
                if isinstance(invoice_payments_widget, str):
                    try:
                        invoice_payments_widget = json.loads(invoice_payments_widget)
                    except json.JSONDecodeError:
                        print(f"Error decoding JSON for invoice_payments_widget in move_id {move_id.id}")
                        continue
                
                if invoice_payments_widget.get('content'):
                    payment_date = invoice_payments_widget['content'][0]['date']
                    if isinstance(payment_date, str):
                        date_payment = datetime.strptime(payment_date, '%Y-%m-%d')
                    else:
            ...
            lines = {
                ...
                'amount_total': '{:.2f}'.format(tax_totals['amount_untaxed']) if tax_totals else '',
