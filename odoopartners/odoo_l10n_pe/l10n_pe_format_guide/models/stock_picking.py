...
class StockPicking(models.Model):
    ...
    cod_client_sucur = fields.Char(
        string="Código Cliente/Sucursal", 
        related="partner_id.cod_client_sucur"
    )
    ...
    def _cal_weight(self):
        for picking in self:
            if picking.move_line_ids and len(picking.move_line_ids) == 1:
                ...
                picking.weight = (move_line.qty_done if picking.state == "done" else move_line.quantity) * move_line.product_id.weight
            else:
                picking.weight = sum((move_line.qty_done if picking.state == "done" else move_line.quantity) * move_line.product_id.weight for move_line in picking.move_line_ids if move_line.state != "cancel")

    @api.depends("move_line_ids.product_id.gross_weight")
    def _compute_gross_weight(self):
        for picking in self:
            picking.gross_weight = sum((move_line.qty_done if self.state == "done" else move_line.quant_id.reserved_quantity) * move_line.product_id.gross_weight for move_line in picking.move_line_ids if move_line.state != "cancel")

    @api.depends("move_line_ids.product_id.volume")
    def _compute_volume(self):
        for picking in self:
            picking.volume = sum((move_line.qty_done if self.state == "done" else move_line.quant_id.reserved_quantity) * move_line.product_id.volume for move_line in picking.move_line_ids if move_line.state != "cancel")
