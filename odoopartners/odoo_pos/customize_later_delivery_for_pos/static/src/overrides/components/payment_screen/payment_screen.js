...
patch(PaymentScreen.prototype, {
    ...
    async _isOrderValid(isForceValidate) {
        ...
        if (this.currentOrder.getShippingDate()) {
            if (this.pos.config.flexible_ship_later) {
                if (
                    !(partner.name && (partner.phone || partner.mobile))
                ) {
                    this.popup.add(ErrorPopup, {
                        title: _t("Celular o teléfono de envío incorrecto"),
                        body: _t("El cliente seleccionado necesita un teléfono o celular."),
                    });
                    return false;
                }
            } else {
