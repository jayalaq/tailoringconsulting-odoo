...
class TOBase(models.AbstractModel):
    ...
    def get_company_tz(self, company=None):
        """
        Return the timezone corresponding to a company.
        :rtype: str
        """
        company = company or self.env.company
        tz = company.partner_id.tz
        if not tz and company.country_id:
            timezones = pytz.country_timezones(company.country_id.code)
            if timezones:
                tz = timezones[0]
        return tz or 'UTC'
    ...
    def convert_local_to_utc(self, dt, force_local_tz_name=None, is_dst=None, naive=False):
        """
        :param dt: an instance of datetime or date object to convert to UTC
        ...
        :rtype: datetime
        """
        given_is_date = (isinstance(dt, date) and not isinstance(dt, datetime))
    ...
    def convert_utc_to_local(self, utc_dt, force_local_tz_name=None, is_dst=None, naive=False):
        ...
        :param utc_dt: datetime or date in UTC
        ...
        if not force_local_tz_name:
            raise ValidationError(_("Local time zone is not defined. You may need to set a time zone in your user's Preferences."))
        given_is_date = (isinstance(utc_dt, date) and not isinstance(utc_dt, datetime))
        if given_is_date:
            utc_dt = self.date_to_datetime(utc_dt)
    ...
    def date_to_datetime(self, date):
        """Convert the given datetime.date value to datetime.datetime
        where time is now time

        :param date: the given datetime.date to convert
        :type datetime.date
        :return: datetime.datetime value that is combined from the given date and the now time
        :rtype: datetime.datetime
        """
        now_time = self.float_hours_to_time(self.time_to_float_hour(fields.Datetime.now()))
        return datetime.combine(date, now_time)
    ...
    def _find_last_date_of_period_from_period_start_date(self, period_name, period_start_date, periods_to_skip=0, cycle_value=1):
        """
        ...
        :param str period_name: the name of the given period which is either 'weekly' or 'monthly' or 'quarterly' or 'biannually' or 'annually'
        :param (datetime.datetime | datetime.date) period_start_date:  the starting date of the period from which the period will be started
        :param int periods_to_skip: the number of periods that will be skipped from the starting date
        ...
        period_name, msg = self._validate_period_name(period_name)
        if not period_name:
            raise ValidationError(msg)
        if period_name == 'hourly':
            if isinstance(period_start_date, date) and not isinstance(period_start_date, datetime):
                raise ValidationError(_("For the hour period, the given date should be in datetime"))
            dt = period_start_date + relativedelta(hours=cycle_value) * (periods_to_skip + 1)
        elif period_name == 'daily':
            dt = period_start_date + relativedelta(days=cycle_value) * (periods_to_skip + 1)
        elif period_name == 'weekly':
            dt = period_start_date + relativedelta(weeks=cycle_value) * (periods_to_skip + 1)
        elif period_name == 'monthly':
            dt = period_start_date + relativedelta(months=cycle_value) * (periods_to_skip + 1)
        elif period_name == 'quarterly':
            dt = period_start_date + relativedelta(months=cycle_value * 3) * (periods_to_skip + 1)
        elif period_name == 'biannually':
            dt = period_start_date + relativedelta(months=cycle_value * 6) * (periods_to_skip + 1)
        else:
            dt = period_start_date + relativedelta(years=cycle_value) * (periods_to_skip + 1)
        ...
        if isinstance(period_start_date, date) and not isinstance(period_start_date, datetime) and isinstance(dt, datetime):
            dt = dt.date()
        return dt
    ...
    def _validate_period_name(self, period_name):
        ...
        msg = ''
        if period_name in compatible_map.keys():
            period_name = compatible_map[period_name]
        if period_name not in compatible_map.values():
    ...
    def find_last_date_of_period(self, period_name, given_date, date_is_start_date=False, cycle_value=1):
        """
        ...
        if date_is_start_date:
            return self._find_last_date_of_period_from_period_start_date(period_name=period_name, period_start_date=given_date, cycle_value=cycle_value)

        dt = self.find_first_date_of_period(period_name, given_date, start_day_offset=0)
        # else
        if period_name == 'hourly':
            dt = dt + relativedelta(hours=cycle_value, microseconds=-1)
        elif period_name == 'daily':
            dt = dt + relativedelta(days=cycle_value, microseconds=-1)
        elif period_name == 'weekly':
            dt = dt + relativedelta(weeks=cycle_value, microseconds=-1)
        elif period_name == 'monthly':
            dt = dt + relativedelta(months=cycle_value, microseconds=-1)
        elif period_name == 'quarterly':
            dt = dt + relativedelta(months=cycle_value * 3, microseconds=-1)
        elif period_name == 'biannually':
            dt = dt + relativedelta(months=cycle_value * 6, microseconds=-1)
        else:
    ...
    def period_iter(self, period_name, dt_start, dt_end, start_day_offset=0, cycle_value=1):
        """
        ...
        :param cycle_value: multiple of the given period_name. For example, period_name is hourly, cycle_value is 3, each period will be 3 hours
        ...
        while period_start_date <= dt_end:
            last_dt = self._find_last_date_of_period_from_period_start_date(period_name=period_name, period_start_date=period_start_date, cycle_value=cycle_value)
            if last_dt > dt_end:
                last_dt = dt_end
            if last_dt not in res:
                res.append(last_dt)
    ...
    @api.model
    def splittor(self, collection, max_rec_in_batch=None, maintain_order=False, flush=True):
        ...
        :param maintain_order: try to maintain the same order as the given recordset by putting the high ordered items
            at the beginning of each sub collection.
        ...
        length = remain_length = len(collection)
        batch_count = math.ceil(length / max_rec_in_batch)
        for i in range(0, batch_count):
            if not maintain_order:
                # Trying to divide the number of elements equally into sub collections.
                # For example, if collection has 20 elements, and max_rec_in_batch is 9,
                # then the result will be 3 sub collections: 2 with 7 elements and 1 with
                # 6 elements, instead of 2 with 9 elements and 1 with 2 elements.
                batch_length = round(remain_length / (batch_count - i))
                sub = collection[length - remain_length: length - remain_length + batch_length]
                remain_length -= batch_length
            else:
                sub = collection[i::batch_count]
            yield sub
    ...
    def get_ratio_between_periods(self, period1, duration1, period2, duration2, given_date=False):
        """
        ...
        :param str period1: the name of the given period which is either 'weekly' or 'monthly' or 'quarterly'
        or 'biannually' or 'annually' or 'hourly' or 'daily'
        :param int duration1: the number of period1 to compare
        :param str period2: the name of the given period which is either 'weekly' or 'monthly' or 'quarterly'
        or 'biannually' or 'annually' or 'hourly' or 'daily'
        :param int duration2: the number of period2 to compare
        :param (datetime.date | datetime.datetime) given_date: the starting date of the period from which the period
        will be started, get current date if value is false

        :return: (float) ratio between periods
        :raise ValidationError: when the passed period_name is invalid
        """
        assert duration1 > 0 and duration2 > 0, "duration1 and duration2 must be greater than 0."
        period1, msg1 = self._validate_period_name(period1)
        period2, msg2 = self._validate_period_name(period2)
        if not period1 or not period2:
            msg = msg1 if not period1 else msg2
            raise ValidationError(msg)
        ...
        if period1 == period2:
            ...
            return (12 if period1 == 'annually' else 1 / 12) * duration1 / duration2
        elif {period1, period2} == {'annually', 'quarterly'}:
            return (4 if period1 == 'annually' else 1 / 4) * duration1 / duration2
        elif {period1, period2} == {'annually', 'biannually'}:
            return (2 if period1 == 'annually' else 1 / 2) * duration1 / duration2
        elif {period1, period2} == {'quarterly', 'monthly'}:
            return (3 if period1 == 'quarterly' else 1 / 3) * duration1 / duration2
        elif {period1, period2} == {'biannually', 'monthly'}:
            return (6 if period1 == 'biannually' else 1 / 6) * duration1 / duration2
        elif {period1, period2} == {'biannually', 'quarterly'}:
            return (2 if period1 == 'biannually' else 1 / 2) * duration1 / duration2
        elif {period1, period2} == {'weekly', 'daily'}:
            return (7 if period1 == 'weekly' else 1 / 7) * duration1 / duration2
        elif {period1, period2} == {'daily', 'hourly'}:
            return (24 if period1 == 'daily' else 1 / 24) * duration1 / duration2
        elif {period1, period2} == {'weekly', 'hourly'}:
            return (24 * 7 if period1 == 'weekly' else 1 / (24 * 7)) * duration1 / duration2
        elif 'hourly' in {period1, period2}:
            tmp_period = period1 if period2 == 'hourly' else period2
            periods_to_skip = duration1 - 1 if period2 == 'hourly' else duration2 - 1
            last_date_period = self._find_last_date_of_period_from_period_start_date(tmp_period, given_date, periods_to_skip)
            range_date_period = self.get_days_between_dates(given_date, last_date_period) + 1
            ratio_between_periods = range_date_period * 24 / duration2 if period2 == 'hourly' else 1 / (range_date_period * 24 / duration1)
        else:
            last_date_period1 = self._find_last_date_of_period_from_period_start_date(period1, given_date, duration1 - 1)
            last_date_period2 = self._find_last_date_of_period_from_period_start_date(period2, given_date, duration2 - 1)

            range_date_period1 = self.get_days_between_dates(given_date, last_date_period1) + 1
            range_date_period2 = self.get_days_between_dates(given_date, last_date_period2) + 1
    ...
    def _get_file_size(self, path):
        ...
        :return: size of the file or directory (byte)
        :rtype: int
        """
        if os.path.isfile(path):
            ...
            return total_size
        raise UserError(_("Cannot get the size of a file or folder"))

    @api.model
    def _set_default_user_rights_minimal(self):
        # TODOs: remove this method on 18.0
        # Disable default user rights propagation
        # When installing a new module, the access of the Default Template User is
        # propagated to any existing employee (introduced at aefb05e). This can be
        # problematic in companies that don't want all their employees to become
        # manager by default.

        # That is not configurable through the interface, manually creating an ICP
        # base_setup.default_user_rights_minimal=True as the way to disable the
        # behaviour default.
        ICP_sudo = self.env['ir.config_parameter'].sudo()
        if not ICP_sudo.search([('key', '=', 'base_setup.default_user_rights_minimal')], limit=1):
            ICP_sudo.create({'key': 'base_setup.default_user_rights_minimal', 'value': True})

    @api.model
    def _set_default_report_print_delay(self):
        ...
        # Add extra time to allow the large page to render
        ICP_sudo = self.env['ir.config_parameter'].sudo()
        if not ICP_sudo.search([('key', '=', 'report.print_delay')], limit=1):
            ICP_sudo.create({'key': 'report.print_delay', 'value': '3000'})
