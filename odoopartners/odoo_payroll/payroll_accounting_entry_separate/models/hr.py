                    ...
                    for line_id in line_ids:
                        if any(slip.employee_id.partner_concept_ids):
                            for partner_concept in slip.employee_id.partner_concept_ids:
                                ...
                                if partner_concept.is_active and line_id['name'] == partner_concept.salary_rule_id.name and check_date_range:
                                    if (partner_concept.is_debit and not partner_concept.is_credit and line_id['debit'] > 0) or (
                                        not partner_concept.is_debit and partner_concept.is_credit and line_id['credit'] > 0) or (
                                            partner_concept.is_debit and partner_concept.is_credit):
                                        line_id['partner_id'] = partner_concept.partner_id.id
                                        break
                                    else:
                                        line_id['partner_id'] = slip.employee_id.work_contact_id.id
                                        break
                                else:
                                    line_id['partner_id'] = slip.employee_id.work_contact_id.id
                        else:
