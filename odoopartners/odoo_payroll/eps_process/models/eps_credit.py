...
class EpsCredit(models.Model):
    ...
    affiliated_workers = fields.Integer(
        string='NÚMERO DE TRABAJADORES AFILIADOS A LA EPS',
        readonly=False
    )
    computable_remuneration_health_input = fields.Float(
        string='REMUNERACIÓN COMPUTABLE APORTE ESSALUD (TODOS AFILIADOS EPS'
    )
    ...
    eps_service_cost = fields.Float(
        string='COSTO DEL SERVICIO EPS DE LOS TRABAJADORES AFILAIDOS (INCLUYE IGV)'
    )
    uit = fields.Float(
        string='UIT',
        readonly=False
    ...
    uit_limit_affiliated_workers = fields.Float(
        string='LÍMITE DE 10% UIT X NÚMERO DE TRABAJADORES AFILIADOS EPS',
        readonly=False
    )
