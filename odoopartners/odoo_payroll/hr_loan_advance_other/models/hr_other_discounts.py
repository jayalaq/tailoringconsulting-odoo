...
class HrOtherDiscounts(models.Model):
    ...
    _description = _("Other discounts")
    ...
    def create(self, values_list):
        ...
        return super(HrOtherDiscounts, self).create(values_list)

    def action_refuse(self):
        for disc in self.discount_lines:
            disc.write({'payment': 0, 'paid': False})
    ...
    def action_payment_anticipated(self):
        ...
        loan_lines_ids = []
        for loan in self.discount_lines.filtered(lambda loan: not loan.paid):
...
class OtherDiscountsLine(models.Model):
    ...
    _description = _("Other Discounts Line")
