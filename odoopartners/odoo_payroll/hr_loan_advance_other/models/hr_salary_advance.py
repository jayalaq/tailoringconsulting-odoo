...
class HrSalaryAdvance(models.Model):
    ...
    def create(self, values_list):
        for values in values_list:
            values['name'] = self.env['ir.sequence'].get('hr.salary.advance.seq') or ' '
        return super(HrSalaryAdvance, self).create(values_list)

    def approve_request(self):
        """This approves the employee salary advance request. """
        self.ensure_one()
        emp_obj = self.env['hr.employee']
        address = emp_obj.browse([self.employee_id.id]).private_street
        if not address:
        ...
        adv = self.advance
        amt = (struct_id.max_percent * self.employee_contract_id.wage) / 100
        ...
        for slip in self.env['hr.payslip'].search([('employee_id', '=', self.employee_id.id)]):
            ...
            if current_month == slip_moth + 1:
                ...
                if current_day - slip_day < struct_id.advance_date:
                    raise RedirectWarning(_('Request can be done after "%s" Days From previous month salary') % struct_id.advance_date)
    ...
    def approve_request_acc_dept(self):
        ...
        for request in self:
            ...
            partner_id = request.employee_id.user_partner_id.id or False
...
class AccountMoveLine(models.Model):
    ...
    def _update_matching_number(self):
        
        for record in self:
            super(AccountMoveLine, record)._update_matching_number()
