from ..reports.bank_report import BankReportTxt
...
class HrMassivePayment(models.Model):
    ...
    future_date = fields.Date(
        string='Fecha futura'
    )
    
    run_time = fields.Selection(
        selection=[
            ('B', '11:00 Horas'),
            ('C', '15:00 Horas'),
            ('D', '19:00 Horas')
    ...
    total_to_pay = fields.Float(
                string="Total to pay", # Etiqueta del campo en la interfaz de usuario
                index=False, # No se crea un índice para este campo
                compute='_compute_total_to_pay', # Se calcula mediante el método indicado
                store=True, # Valor calculado se almacena en BD para mejorar el rendimiento y estamos usando un api.depends para asegurar el re-cómputo
                readonly=True # El usuario no puede modificar este campo directamente porque se calcula en automático.
    )

    @api.depends('payslip_ids')
    def _compute_total_to_pay(self):
        for payslip in self:
            total_pay = sum(line.net_other for line in payslip.payslip_ids)
            payslip.total_to_pay = total_pay
    
    def _compute_display_name(self):
        for payment in self:
            payment.display_name = '{}'.format(payment.payment_date.strftime('%d/%m/%Y') or '')
    ...
    def get_data_scotiabank_wage(self, data):
        ...
        if employees_no_accounts:
            error = 'Los siguientes trabajadores no tienen número de cuenta en el campo bank_account_id de su ficha de empleado, y si lo tienen, entonces no tienen un número de cuenta a su nombre que coincida con el tipo (Sueldo o CTS) y la Moneda de este registro de pago masivo:\n%s'
    ...
    def get_data_bbva_wage(self, data):
        ...
        row1_part9 = self.complete_str_data(' ', 20, 'rigth')
        row1_part10 = '000000000000000'
        row1_part11 = self.complete_str_data(' ', 50, 'rigth')
        row1 = row1_part1 + row1_part2 + row1_part3 + row1_part4 + row1_part5 + row1_part6 + row1_part7 + \
               row1_part8 + 'N' + row1_part9 + row1_part10 + row1_part11
        data['row1'] = '700' + row1
    ...
    def get_partner_bank_by_code(self, employee, code, operator, acc_type):
        ...
        domain = [
            ('partner_id', '=', employee.bank_account_id.partner_id.id),
            ('bank_id', '!=', False),
