...
from ..reports.sire_sale_accept_proposed import SireSaleAcceptProposedXlsx, SireSaleAcceptProposedTxt
from ..reports.sire_sale_replace_proposed import SireSaleReplaceProposedXlsx, SireSaleReplaceProposedTxt
from ..reports.sire_sale_subsequent_adjustments import SireSaleSubsequentAdjustmentsXlsx, SireSaleSubsequentAdjustmentsTxt
from ..reports.sire_sale_general_format import SireSaleGeneralFormatXlsx, SireSaleGeneralFormatTxt


class SireSaleWizard(models.TransientModel):
    _name = 'sire.sale.wizard'
    _inherit = 'sire.sale.base'
    _description = 'Sire Sale Wizard'
    ...
    opportunity_code = fields.Selection(
        selection=[
            ('01', '[01] RVIE - Cuando acepta la propuesta'),
            ('02', '[02] RVIE - Cuando reemplaza la propuesta'),
            ('03', '[03] RVIE - Cuando realiza ajustes posteriores'),
    ...
    def _generate_xlsx_file(self, processed_results):
        if self.opportunity_code == '01':
            xlsx_report = SireSaleAcceptProposedXlsx(self, processed_results)
        elif self.opportunity_code == '02':
            xlsx_report = SireSaleReplaceProposedXlsx(self, processed_results)
        elif self.opportunity_code == '03':
            xlsx_report = SireSaleSubsequentAdjustmentsXlsx(self, processed_results)
        elif self.opportunity_code == '04':
            xlsx_report = SireSaleGeneralFormatXlsx(self, processed_results)
        else:
    ...
    def _generate_txt_file(self, processed_results):
        if self.opportunity_code == '01':
            txt_report = SireSaleAcceptProposedTxt(self, processed_results)
        elif self.opportunity_code == '02':
            txt_report = SireSaleReplaceProposedTxt(self, processed_results)
        elif self.opportunity_code == '03':
            txt_report = SireSaleSubsequentAdjustmentsTxt(self, processed_results)
        elif self.opportunity_code == '04':
            txt_report = SireSaleGeneralFormatTxt(self, processed_results)
        else:
            raise ValidationError('Por favor, se necesita un código de oportunidad para generar el reporte, gracias.')
        return {
    ...
    def _return_view_sire_sale(self):
        wizard_form_id = self.env.ref('l10n_pe_sire_sunat.sire_sale_wizard_form_view').id
        return {
            'type': 'ir.actions.act_window',
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'sire.sale.wizard',
