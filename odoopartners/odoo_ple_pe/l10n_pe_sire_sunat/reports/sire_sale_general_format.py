...
class SireSaleGeneralFormatXlsx:
    ...
    def _write_rows_report(workbook, worksheet, processed_results):
        ...
        for dict_result in processed_results:
            ...
            worksheet.write(row, 8, dict_result['invoice_correlative'].lstrip('0'), style_content)
            ...
            worksheet.write(row, 21, dict_result['s_base_ivap'], style_number_two_decimal)
            worksheet.write(row, 22, dict_result['s_tax_ivap'], style_number_two_decimal)
            worksheet.write(row, 23, dict_result['s_tax_icbp'], style_number_two_decimal)
...
class SireSaleGeneralFormatTxt:
    ...
    def _get_template_report():
        return '{period}|{invoice_name}|' \
            ...
            '{s_base_oe}|{s_base_ou}|{s_tax_isc}|{s_base_ivap}|' \
            '{s_tax_ivap}|{s_tax_icbp}|{s_tax_other}|{amount_total}|' \
    ...
    def _write_template_report(template, processed_results):
        ...
        for dict_result in processed_results:
            content += template.format(
                ...
                invoice_correlative=dict_result['invoice_correlative'].lstrip('0'),
