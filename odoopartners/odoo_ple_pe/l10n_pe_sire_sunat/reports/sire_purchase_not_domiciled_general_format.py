...
class SirePurchaseNotDomiciledGeneralFormatXlsx:
    ...
    def _write_headers_report(workbook, worksheet):
        ...
        worksheet.write(0, 5, 'Tipo CP/Doc', style_header)
        worksheet.write(0, 6, 'Serie del CDP', style_header)
        worksheet.write(0, 7, 'Nro CP o Doc', style_header)
        worksheet.write(0, 8, 'Val Adquisiciones', style_header)
        worksheet.write(0, 9, 'Otros', style_header)
        worksheet.write(0, 10, 'Total CP', style_header)
        worksheet.write(0, 11, 'Tipo CP CF', style_header)
        worksheet.write(0, 12, 'Serie CP CF', style_header)
        worksheet.write(0, 13, 'Año', style_header)
        worksheet.write(0, 14, 'Nro CP o Doc. CF', style_header)
        worksheet.write(0, 15, 'Monto Ret', style_header)
        worksheet.write(0, 16, 'Moneda', style_header)
        worksheet.write(0, 17, 'Tipo de Cambio', style_header)
        worksheet.write(0, 18, 'País', style_header)
        worksheet.write(0, 19, 'Apellidos Nombres/ \nRazon Social del sujeto', style_header)
        worksheet.write(0, 20, 'Domicilio', style_header)
        worksheet.write(0, 21, 'ID Sujeto', style_header)
        worksheet.write(0, 22, 'ID Beneficiario', style_header)
        worksheet.write(0, 23, 'Apellidos Nombres/ \nRazon Social del \nbeneficiario', style_header)
        worksheet.write(0, 24, 'País beneficiario', style_header)
        worksheet.write(0, 25, 'Vínculo', style_header)
        worksheet.write(0, 26, 'Rta Bta', style_header)
        worksheet.write(0, 27, 'Deduc/Costo', style_header)
        worksheet.write(0, 28, 'Rta Neta', style_header)
        worksheet.write(0, 29, 'Tasa', style_header)
        worksheet.write(0, 30, 'Impto', style_header)
        worksheet.write(0, 31, 'Convenio', style_header)
        worksheet.write(0, 32, 'Exon.', style_header)
        worksheet.write(0, 33, 'Tipo Rta', style_header)
        worksheet.write(0, 34, 'o Mod Serv', style_header)
        worksheet.write(0, 35, 'Art. 76', style_header)
        worksheet.write(0, 36, 'Estado', style_header)
        worksheet.write(0, 37, '', style_header)
    ...
    def _write_rows_report(workbook, worksheet, processed_results):
        ...
        for dict_result in processed_results:
            ...
            worksheet.write(row, 5, dict_result['document_type_code'], style_content)
            worksheet.write(row, 6, ref_serie, style_content)
            worksheet.write(row, 7, ref_correlative.lstrip('0'), style_content)
            worksheet.write(row, 8, dict_result['p_base_ng'], style_number_two_decimal)
            worksheet.write(row, 9, '', style_content)
            worksheet.write(row, 10, dict_result['p_base_ng'], style_number_two_decimal)
            worksheet.write(row, 11, dict_result['inv_type_document_code'], style_content)
            worksheet.write(row, 12, dict_result['inv_serie'], style_content)
            worksheet.write(row, 13, dict_result['inv_year_dua_dsi'], style_content)
            worksheet.write(row, 14, dict_result['inv_correlative'], style_content)
            worksheet.write(row, 15, dict_result['inv_retention_igv'], style_number_two_decimal)
            worksheet.write(row, 16, dict_result['currency_name'], style_content)
            worksheet.write(row, 17, dict_result['exchange_rate'], style_number_three_decimal)
            worksheet.write(row, 18, dict_result['country_code'], style_content)
            worksheet.write(row, 19, dict_result['partner_name'])
            worksheet.write(row, 20, domicile, style_content)
            worksheet.write(row, 21, dict_result['partner_vat'])
            worksheet.write(row, 22, '', style_content)
            worksheet.write(row, 23, '', style_content)
            worksheet.write(row, 24, '', style_content)
            worksheet.write(row, 25, dict_result['linkage_code'], style_content)
            worksheet.write(row, 26, float(dict_result['hard_rent']), style_number_two_decimal)
            worksheet.write(row, 27, float(dict_result['deduccion_cost']), style_number_two_decimal)
            worksheet.write(row, 28, float(dict_result['neto_rent']), style_number_two_decimal)
            worksheet.write(row, 29, float(dict_result['retention_rate']), style_number_two_decimal)
            worksheet.write(row, 30, float(dict_result['tax_withheld']), style_number_two_decimal)
            worksheet.write(row, 31, dict_result['cdi'], style_content)
            worksheet.write(row, 32, dict_result['exoneration_nodomicilied_code'], style_content)
            worksheet.write(row, 33, dict_result['type_rent_code'], style_content)
            worksheet.write(row, 34, dict_result['taken_code'], style_content)
            worksheet.write(row, 35, dict_result['application_article'], style_content)
            worksheet.write(row, 36, '9', style_content)
            row += 1
...
class SirePurchaseNotDomiciledGeneralFormatTxt:
    def __init__(self, obj, processed_results):
        self.obj = obj
        self.processed_results = processed_results
    ...
    def _get_template_report():
        return '{period}|{invoice_name}|{invoice_line_correlative}|{invoice_date}|' \
            ...
            '{taken_code}|{application_article}|9|\r\n'
    ...
    def _write_template_report(template, processed_results):
        ...
        for dict_result in processed_results:
            ...
            content += template.format(
                ...
                ref_serie=ref_serie,
                ref_correlative=ref_correlative.lstrip('0'),
    ...
    def get_filename(self):
        return 'LE{}{}{}00080500{}{}{}12{}.txt'.format(
            ...
            self.obj.correlative.lstrip('0') if self.obj.correlative else ''
        )
