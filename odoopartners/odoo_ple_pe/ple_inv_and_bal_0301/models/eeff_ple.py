...
class EEFFPLE(models.Model):
    ...
    @api.model
    def automed_update_id_external(self):
        for record in data_id:
            code_id = self.env['eeff.ple'].search([('code' ,'=', record[1]),('eeff_type','=',record[2])],limit=1)
            eeff_ple = self.env['ir.model.data'].search([('model','=','eeff.ple'),('res_id','=',code_id.id)],limit=1)
            
            if eeff_ple:
                eeff_ple.write({ 'name':record[0] })
            else:
                eeff_ple.create({
                    'name':record[0],
                    'model':'eeff.ple',
