...
class WizardReportTxtPLE(models.TransientModel):
    ...
    def generate_318_data_txt(self, rpl):
        ...
        if len(filter_rpl) >= 3:
            data_txt_dict.append({'code': '3D0405', 'hierarchy': 'PADRE TOTAL', 'balance': filter_rpl[1]['columns'][0]['name'].replace(',', '')})
            data_txt_dict.append({'code': '3D0402', 'hierarchy': 'SIN PADRE', 'balance': filter_rpl[0]['columns'][0]['name'].replace(',', '')})
            data_txt_dict.append({'code': '3D04ST', 'hierarchy': 'SIN PADRE', 'balance': filter_rpl[-1]['columns'][0]['name'].replace(',', '')})
        return data_txt_dict
    ...
    def generate_data(self):
        ...
        for line in report_lines:
            if line['name'][0].isdigit():
                code = line['name'].split(' ')[0]
                line_id = self.env['account.account'].search([('code', '=', code)], limit=1)
                balance = line['columns'][0]['name'].replace(',', '')
