...
class PleDiary(models.Model):
    ...
    def action_generate_excel(self):
        ...
        def get_translated_field_sql(table_alias, field_name, length=None):
            lang = self.env.user.lang or get_lang(self.env).code
            field_expr = f"{table_alias}.{field_name}"

            if length:
                return f"""COALESCE(
                    trim(substring({field_expr}->>'{lang}', 1, {length})),
                    trim(substring({field_expr}->>'en_US', 1, {length})),
                    ''
                )"""
            else:
                return f"COALESCE({field_expr}->>'{lang}', {field_expr}->>'en_US', '')"
        ...
        """.format(
            account_name_sql=get_translated_field_sql('account_account', 'name', 99),
            group_name_sql=get_translated_field_sql('account_group', 'name', 59),
            company_id=self.company_id.id
        )
