...
from ..reports.report_inv_bal_14 import ReportInvBal14Excel, ReportInvBal14Txt
import re


class PleInvBal14(models.Model):
    _inherit = 'ple.report.inv.bal.one'

    xls_filename_314 = fields.Char(string='Filaname_0314 Excel')
    xls_binary_314 = fields.Binary(string='Reporte Excel')
    txt_filename_314 = fields.Char(string='Filaname_0314 .txt')
    txt_binary_314 = fields.Binary(string='Reporte .TXT 3.14')
    pdf_filename_314 = fields.Char(string='Filaname_0314 .pdf')
    pdf_binary_314 = fields.Binary(string='Reporte .PDF 3.14')

    line_ids_314 = fields.One2many(comodel_name='ple.report.inv.bal.line.14', inverse_name='report_id')

    def action_generate_excel(self):
        super().action_generate_excel()

        data = self.generate_data_report_314()

        report_xls = ReportInvBal14Excel(self, data)
        values_content_xls = report_xls.get_content()
        self.xls_binary_314 = base64.b64encode(values_content_xls)
        self.xls_filename_314 = report_xls.get_filename()

        report_txt = ReportInvBal14Txt(self, data)
        values_content_txt = report_txt.get_content()
        self.txt_binary_314 = base64.b64encode(values_content_txt.encode() or '\n'.encode())
        self.txt_filename_314 = report_txt.get_filename()

        report_name = "ple_inv_and_bal_0311_0314.action_print_report_pdf_0314"
        pdf = self.env.ref(report_name)._render_qweb_pdf('ple_inv_and_bal_0311_0314.print_report_pdf_0314',self.id)[0]
        self.pdf_binary_314 = base64.encodebytes(pdf)
        self.pdf_filename_314 = f"Libro_Benef_socia_de_los_empleados_{self.date_end.strftime('%Y%m')}.pdf"

        self.line_ids_314.unlink()

    def _set_values_14(self, obj_move_line):
    ...
    def generate_data_report_314(self):
        data_account = {}
        objs = self.env['account.account'].search([
            ('group_id.code_prefix_start', '=', '41'),
            ('ple_selection', '=', 'assets_book_employee_benefits')
        ])
        for obj_account in objs:
            ...
            if not obj_account.reconcile:
                ...
                if list_move_line:
                    ...
                    values.update(self._set_values_14(list_move_line[0]))
                ...
                if list_ml_reconcile:
                    ...
                    for obj_ml in list_ml_reconcile:
                        ...
                        values.update(self._set_values_14(obj_ml))
                ...
                if list_ml_unreconcile_filter:
                    ...
                    for list_ml in list_unreconcile_set:
                        ...
                        data_d.update(self._set_values_14(list_ml[0]))
                        for i in range(len(list_ml)):
                            ...
                            if data_d['move_line_id'] > list_ml[i].id:
                                ...
                                data_d.update(self._set_values_14(list_ml[i]))
                ...
                for obj_ml_zero in list_ml_zero:
                    ...
                    values.update(self._set_values_14(obj_ml_zero))
                    data_account[name_account].append(values)
        return data_account


class PleInvBalLines14(models.Model):
    _name = 'ple.report.inv.bal.line.14'
    _description = 'Reporte 3.14 - Líneas'
