...
class TestPleInvAndBal0302(TransactionCase):

    @classmethod
    def setUpClass(self):
        super(TestPleInvAndBal0302, self).setUpClass()
        ...
        self.ple_inv_bal_one_line_initial_balances = self.env[
            "ple.inv.bal.one.line.initial.balances"
        ]
        self.ple_inb_bal_one_line_final_balances = self.env[
            "ple.inv.bal.one.line.final.balances"
        ]
        self.random_set = {
            "a": "123456",
            "b": "567789",
    ...
    def test_ple_report_inv_val_one_id(self):
        ...
        self.assertEqual(
            ple_inv_bal_one_temp.xls_filename,
            f"Libro_Efectivo y Equivalente de efectivo_{year}{month}.xlsx",
        )
        self.assertEqual(
            ple_inv_bal_one_temp.pdf_filename,
            f"Libro_Efectivo y equivalente de efectivo_{year}{month}.pdf",
        )

    def create_initial_balances(self):
        common_obj = self.common_create_object("b", "e")
        temp_ple_initial_balances = self.ple_inv_bal_one_line_initial_balances.create(
            common_obj
        )
    ...
    def create_final_balances(self):
        ...
        temp_ple_final_balances = self.ple_inb_bal_one_line_final_balances.create(
            common_obj
        )
    ...
    def test_ple_report_inv_val_one_balances(self):
        ...
        ple_inv_bal_one_temp.write(
            {
                "line_initial_ids": [(6, 0, ple_inv_initial_balances.ids)]
            }
        )
        ple_inv_bal_one_temp.write(
            {
                "line_final_ids": [(6, 0, ple_inv_final_balances.ids)]
            }
        )
        ple_inv_initial_balances.write(
            {
                "ple_report_inv_val_one_id": ple_inv_bal_one_temp.id
            }
        )
        ple_inv_final_balances.write(
            {
                "ple_report_inv_val_one_id": ple_inv_bal_one_temp.id
            }
        )
        self.assertRaises(
            AccessError, 
            ple_inv_bal_one_temp.action_compute_initial_balance_302()
        )
        self.assertRaises(
            AccessError, 
            ple_inv_bal_one_temp.capture_initial_balances_id()
        )
