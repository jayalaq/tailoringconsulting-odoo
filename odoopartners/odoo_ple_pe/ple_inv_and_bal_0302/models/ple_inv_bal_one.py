...
class PleInvBal1One(models.Model):
    ...
    def name_get(self):
        ...
        for rec in self:
            res.append(
                (
                    rec.id,
                    f"{rec.date_start.strftime('%d/%m/%Y')} - {rec.date_end.strftime('%d/%m/%Y')}",
                )
            )
        return res

    def action_generate_excel(self):
        def get_translated_column_name(model_name, table_name_alias):
            if self.pool[model_name].name.translate:
                lang = self.env.user.lang or get_lang(self.env).code
                return f"COALESCE({table_name_alias}.name->>'{lang}', {table_name_alias}.name->>'en_US')"
            else:
                return f'{table_name_alias}.name'
        ...
        bank_account_name = get_translated_column_name('account.account', 'lines')
        query = f"""
            SELECT
        ...
        try:
            ...
            raise ValidationError(
                f"Error al ejecutar queries, comunciar al administrador: \n {error}"
            )
    ...
    def get_excel_data(self, data_lines):
        ...
        pdf = self.env.ref(report_name)._render_qweb_pdf(
            "ple_inv_and_bal_0302.print_status_finance_0302", self.id
        )[0]
