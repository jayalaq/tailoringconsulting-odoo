...
class ReportInvBalEightTxt(object):
    ...
    def get_content(self):
        ...
        template = '{name}|{name_document_string}|{correlative}|{type_document_transmitter}|{number_document_transmitter}' \
                   ...
                   '{total_title_provision}|{operation_status}|\r\n'
        ...
        for i in range(len(self.data_1)):
            ...
            data.append(join)
        for value in data:
            if value['title_unit_value'] is None:
                raise UserError("Falta definir el campo Asset/Intangible para el Reporte 3.8 en la ruta Contabilidad/Contabilidad/Apuntes Contables")
            try:
                name_document_string = ''.join(char for char in value['document_name'] if char.isalnum())
                raw += template.format(
                    name=value['name'],
                ...
                raise UserError("Error de formato en los datos procesados en el Reporte 3.8")
