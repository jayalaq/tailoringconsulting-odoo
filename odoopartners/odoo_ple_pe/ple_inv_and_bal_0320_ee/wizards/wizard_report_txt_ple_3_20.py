...
class WizardReportTxtPLEOld(models.TransientModel):
    ...
    _description = 'Wizard Report TXT PLE 3.20 Old'


class WizardReportTxtPLE(models.TransientModel):
    _name = 'wizard.report.txt.ple.3.20'
    _description = 'Wizard Report TXT PLE 3.20'
    ...
    def action_generate_report_txt_ple(self):
        eerr_report = self.env['account.report'].browse(self.report_id)
        eerr_options = self._get_report_options(eerr_report)
        
        pre_report_lines = self._prepare_report_lines(eerr_report, eerr_options)
    ...
    def _get_report_options(self, eerr_report):
        options = eerr_report.get_options()
        options['date']['date_from'] = self.date_start.strftime('%Y-%m-%d')
        options['date']['date_to'] = self.date_end.strftime('%Y-%m-%d')
        return options
    
    def _prepare_report_lines(self, eerr_report, eerr_options):
        pre_report_lines = []

        eerr_report_lines = eerr_report._get_lines(eerr_options)
        
        for eerr_report_line in eerr_report_lines:
            model, line_id = eerr_report._parse_line_id(eerr_report_line.get('id'))[-1][1:]
            ...
            if report_line.re_item and report_line.re_item.eeff_type == '3.20':
                pre_report_lines.append({
                    'sequence': report_line.re_item.sequence,
                    'period': self.date_end.strftime('%Y%m%d'),
                    'catalog_code': self.financial_statements_catalog,
                    'eerr_ple_id': report_line.re_item.id,
                    'parent_id': report_line.re_item.id,
                    'financial_code': report_line.re_item.code,
                    'balance': eerr_report_line['columns'][0]['no_format'],
                    'state': 1
                })
    ...
    def _process_report_lines(self, pre_report_lines):
        ...
        for pre_report_line in pre_report_lines:
            self._check_key_in_dicts(pre_report_line['eerr_ple_id'], dict_report_lines, pre_report_line)
    ...
    def _check_parent_lines(self, eerr_ple_id, balance, dict_data):
        if isinstance(eerr_ple_id, int):
            eeff_ple_id = self.env['eeff.ple'].search([('id', '=', eerr_ple_id)], limit=1)
        else:
            eeff_ple_id = eerr_ple_id
        ...
        report_lines = [
            {
                ...
                'balance': round(float(sorted_pro_report_line['balance']), 2),
