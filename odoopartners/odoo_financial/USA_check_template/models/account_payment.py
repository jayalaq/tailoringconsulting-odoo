...
class MyAccountPayment(models.Model):
    _inherit = 'account.payment'

    def _check_get_pages_usa(self):
        stub_pages = self._check_make_stub_pages_usa() or [False]
        pages = []
        for i, p in enumerate(stub_pages):
            pages.append(self._check_build_page_info(i, p))
        return pages

    def _check_make_stub_pages_usa(self):
        ...
        def prepare_vals(invoice, partials):
            ref = invoice.ref
            type_bill = self.sudo().env["ir.config_parameter"].get_param("USA.type_bill")
